...
static void esp_bh1750_sensor_handler(float illuminance)
{
  ...
  esp_zb_lock_acquire(portMAX_DELAY);
  esp_zb_zcl_set_attribute_val(
      HA_ESP_SENSOR_ENDPOINT, ESP_ZB_ZCL_CLUSTER_ID_ILLUMINANCE_MEASUREMENT, ESP_ZB_ZCL_CLUSTER_SERVER_ROLE, ESP_ZB_ZCL_ATTR_ILLUMINANCE_MEASUREMENT_MEASURED_VALUE_ID, &ill_value,
      false);
  esp_zb_lock_release();
}
...
static esp_zb_cluster_list_t *custom_sensor_clusters_create(esp_zb_temperature_sensor_cfg_t *temperature_sensor,
                                                            esp_zb_humidity_meas_cluster_cfg_t *humidity_sensor,
                                                            esp_zb_illuminance_meas_cluster_cfg_t *illuminance_sensor)
{
  ...
  ESP_ERROR_CHECK(esp_zb_cluster_list_add_illuminance_meas_cluster(
      cluster_list, esp_zb_illuminance_meas_cluster_create(illuminance_sensor), ESP_ZB_ZCL_CLUSTER_SERVER_ROLE));
    ...
    esp_zb_illuminance_meas_cluster_cfg_t *illuminance_sensor)
{
  ...
  esp_zb_endpoint_config_t endpoint_config = {
      .endpoint = endpoint_id, .app_profile_id = ESP_ZB_AF_HA_PROFILE_ID, .app_device_id = ESP_ZB_HA_CUSTOM_ATTR_DEVICE_ID, .app_device_version = 0};
  esp_zb_ep_list_add_ep(ep_list, custom_sensor_clusters_create(temperature_sensor, humidity_sensor, illuminance_sensor), endpoint_config);
  ESP_LOGI(TAG, "%s", "Created endpoint!");
  return ep_list;
}

static void esp_zb_task(void *pvParameters)
{
  ESP_LOGI(TAG, "%s", "Started main task!");
  ...
  // Init BH1750 to avoid issue where value updates before endpoint is ready
  ESP_LOGI(TAG, "%s", "Initializing BH1750");
  ESP_ERROR_CHECK(bh1750_driver_init(ESP_TEMP_SENSOR_UPDATE_INTERVAL, esp_bh1750_sensor_handler));
...
void app_main(void)
{
  ...
  // Init I2C
  ESP_LOGI(TAG, "%s", "Initializing I2C");
  ESP_ERROR_CHECK(i2c_init());

  // Init AHT20
  ESP_LOGI(TAG, "%s", "Initializing AHT20");
  ESP_ERROR_CHECK(aht_driver_init(ESP_TEMP_SENSOR_UPDATE_INTERVAL, esp_aht20_sensor_handler));

  // Init BH1750 delay init until zb task is started
  //ESP_LOGI(TAG, "%s", "Initializing BH1750");
  //ESP_ERROR_CHECK(bh1750_driver_init(ESP_TEMP_SENSOR_UPDATE_INTERVAL, esp_bh1750_sensor_handler));
